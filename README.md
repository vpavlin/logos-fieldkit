...
| **Download hotspot** | A WiFi AP (`LogosFieldNode`) + HTTP server offering Basecamp (Linux/macOS), **two in-app package repos** (our mesh modules + an on-node mirror of the official Logos modules — delivery, chat, storage, wallet, …), the node binary + circuits, docs, and an offline library. |
